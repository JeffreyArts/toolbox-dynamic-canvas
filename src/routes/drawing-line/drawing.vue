            ...
            <div class="options">
                <div class="option-group" name="Shapes">
                    <div class="option __isGroup">
                        <span>
                            <canvas class="shape-icon" id="shape-line" @click="selectShape('DCLine')" :class="[selectedShape == 'DCLine' ? '__isSelected' : '']"></canvas>
                        </span>
                        <span>
                ...
                <div class="option-group" name="Fill" v-if="['DCRectangle', 'DCSquare', 'DCCircle', 'DCEllipse', 'DCLine'].includes(selectedShape)">
                    <div class="option __isGroup">
                ...
                <div class="option-group" name="Stroke" v-if="['DCRectangle', 'DCSquare', 'DCCircle', 'DCEllipse', 'DCLine'].includes(selectedShape)">
                    <div class="option __isGroup">
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            mouseDown: false,
            newShape: undefined as DCCircle | DCEllipse | DCRectangle | DCSquare | DCImage | DCLine | undefined,
            selectedShapeObject: undefined as DCCircle | DCEllipse | DCRectangle | DCSquare | DCImage | DCLine | undefined,
            shapes: [] as Array<DCCircle | DCEllipse | DCRectangle | DCSquare | DCImage | DCLine>,
            keys: {} as Record<string, boolean>,
            isTransparent: false,
            isDrawingLine: false,
            currentLine: undefined as DCLine | undefined,
            previewLine: undefined as DCLine | undefined,
            startPoint: undefined as { x: number, y: number } | undefined,
            car: {
                speed: 0,
                maxSpeed: 16,
                acceleration: 0.2,
    ...
    methods: {
        getCanvasCoordinates(_event: MouseEvent) {
        ...
        onMouseDown(_event: MouseEvent) {
            const coords = this.getCanvasCoordinates(_event)
            
            if (this.selectedShape === "DCLine") {
                this.mouseDown = true
                this.startPoint = coords
                this.startNewLine(coords)
            } else {
                this.mouseDown = true
        ...
        onMouseUp(_event: MouseEvent) {
            ...
            if (this.selectedShape === "DCLine" && this.startPoint) {
                const coords = this.getCanvasCoordinates(_event)
                
                // Verwijder de preview lijn
                if (this.previewLine) {
                    this.dynamicCanvas?.layers.pop()
                    this.previewLine = undefined
                }

                // Controleer of de muis binnen het canvas is
                if (coords.x >= 0 && coords.x <= this.dynamicCanvas!.canvas.width &&
                    coords.y >= 0 && coords.y <= this.dynamicCanvas!.canvas.height) {
                    // Maak de definitieve lijn
                    const line = new DCLine(this.dynamicCanvas!.canvas, {
                        points: [
        ...
        onMouseMove(_event: MouseEvent) {
            ...
            if (this.selectedShape === "DCLine" && this.startPoint) {
                // Verwijder de preview lijn als die bestaat
                if (this.previewLine) {
                    this.dynamicCanvas?.layers.pop()
                }

                // Maak een nieuwe preview lijn
                this.previewLine = new DCLine(this.dynamicCanvas!.canvas, {
                    points: [
                        { x: this.startPoint.x, y: this.startPoint.y },
                        { x: coords.x, y: coords.y }
                    ],
                    stroke: { ...this.stroke, color: this.stroke.color + "80" }, // 50% transparant
                    closed: false,
                    fill: "transparent"
                })
                this.dynamicCanvas?.layers.push(this.previewLine)
            } else if (this.newShape) {
                if (this.newShape instanceof DCCircle) {
                    this.drawCircle({x: coords.x, y: coords.y, center: !!_event.altKey}) 
                } else if (this.newShape instanceof DCSquare) {
                    this.drawSquare({x: coords.x, y: coords.y, center: !!_event.altKey}) 
                } else if (this.newShape instanceof DCRectangle) {
                    this.drawRectangle({x: coords.x, y: coords.y, center: !!_event.altKey}) 
                } else if (this.newShape instanceof DCEllipse) {
                    this.drawEllipse({x: coords.x, y: coords.y, center: !!_event.altKey}) 
                } else if (this.newShape instanceof DCImage) {
                    this.drawImage({x: coords.x, y: coords.y, center: !!_event.altKey}) 
                }
            }
        },
        ...
        drawShapeIcon(type: string) {
            ...
            if (type == "line") {
                canvas = document.getElementById("shape-line") as HTMLCanvasElement
            } else if (type == "circle") {
        ...
        startNewLine(pos: { x: number, y: number }) {
            if (!this.dynamicCanvas) return
            
            this.isDrawingLine = true
            this.currentLine = new DCLine(this.dynamicCanvas.canvas, {
                points: [{ x: pos.x, y: pos.y }],
                stroke: this.stroke,
                closed: true,
                fill: this.fillColor
            })
            this.dynamicCanvas.layers.push(this.currentLine)
        },
        addPointToLine(pos: { x: number, y: number }) {
